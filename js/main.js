...
function enviarDadosComImagens() {
    ...
    if (files.length > 0) {
        const imagensPromises = Array.from(files).map((file) => {
            const imagemRef = firebase.storage().ref().child(numeroCapitulo + '/' + file.name);
        ...
        Promise.all(imagensPromises)
            .then((urls) => {
                ...
                const dados = {
                    capitulo: document.getElementById('num-cap').value,
    ...
    mangaRef.orderByChild('capitulo').on('value', (snapshot) => {
        ...
        if (arrayDeDados) {
            ...
            for (const cap in arrayDeDados) {
                    ...
                    const chave = arrayDeDados[cap].id
                    criarHtmlParaTodosCaps(numeroCapitulo, chave);
...
function criarHtml(primeiraImagemCapitulo, chave) {
    const divRow = $('#cards-cap');
    ...
    const cardElementViewA = $('<a>', {
        ...
        'data-target': '#mangaModal', 'data-dismiss': 'modal', 'data-id': chave
...
function criarHtmlParaTodosCaps(numeroCapitulo, chaveCap) {
    const divClassAnimeDatails = $('#manga_all_caps');
    const tagA = $('<a>', {
        'type': 'button', 'data-toggle': "modal",
        'data-target': '#mangaModal', 'data-dismiss': 'modal', 'data-id': chaveCap
    }).text('Cap '+ numeroCapitulo);

    divClassAnimeDatails.append(tagA)
}
